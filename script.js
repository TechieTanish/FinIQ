...
// ----- Toasts top-center with click sound -----
...
async function createTransaction({ amount, type, category, mode, notes }) {
  ...
  if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Failed to create transaction'); }
  return res.json();
}
async function updateTransaction(id, data) {
  ...
  if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Failed to update transaction'); }
  return res.json();
}
async function deleteTransactionById(id) {
  const res = await fetch(`${API}/transactions/${id}`, { method: 'DELETE' });
  if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Failed to delete transaction'); }
  return res.json();
}
...
async function saveBudget(newB) {
  const res = await fetch(`${API}/budget`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ budget: newB })
  });
  if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Failed to save budget'); }
  await loadBudget();
...
function computeMonthlyStats() {
  ...
  for (const tx of transactions) {
    ...
    if (tx.type === "income") inc += Number(tx.amount) || 0; else if (tx.type === "expense") exp += Number(tx.amount) || 0;
  }
  return { inc, exp };
}
...
function openPopup(type) {
  ...
  popup.classList.add("open"); popup.setAttribute("aria-hidden", "false"); errorMsg.textContent = "";
}
document.getElementById("close-popup").addEventListener("click", () => { popup.classList.remove("open"); popup.setAttribute("aria-hidden", "true"); });
function loadCategories(type){
  const catSelect = document.getElementById("category"); catSelect.innerHTML = "";
  (categories[type] || []).forEach(c=>{ const opt = document.createElement("option"); opt.value = c.value; opt.textContent = c.text; catSelect.appendChild(opt); });
}
...
async function onSaveClick(){
  ...
  } catch(e){ errorMsg.textContent = e.message || "Failed to save"; showToast("Save failed", "error"); }
}

// Delete
async function handleDelete(id){ try { await deleteTransactionById(id); await loadTransactions(); showToast("Transaction deleted", "success"); } catch(e){ showToast(e.message || "Failed to delete", "error"); } }

// Edit
function startEdit(id){
  const tx = transactions.find(t => t.id === id); if(!tx) return;
  currentType = tx.type; loadCategories(tx.type);
  document.getElementById("amount").value=tx.amount; document.getElementById("category").value=tx.category; document.getElementById("mode").value=tx.mode; document.getElementById("notes").value=tx.notes || "";
  ...
  saveBtn.onclick = async () => {
    ...
    try {
      await updateTransaction(id, { amount: amt, type: currentType, category: document.getElementById("category").value, mode: document.getElementById("mode").value, notes: document.getElementById("notes").value });
      await loadTransactions();
      popup.classList.remove("open"); popup.setAttribute("aria-hidden", "true"); resetSaveBtn(); showToast("Transaction updated", "success");
    } catch(e){ errorMsg.textContent = e.message || "Failed to update"; showToast("Update failed", "error"); }
  };
}
function resetSaveBtn(){ const saveBtn = document.getElementById("save-transaction"); saveBtn.onclick = null; saveBtn.addEventListener("click", onSaveClick, { once: false }); }
resetSaveBtn();
...
function setDefaultDates() {
  ...
  fromInput.valueAsDate = first; toInput.valueAsDate = now; dateFrom = new Date(first); dateTo = new Date(now);
}
fromInput.addEventListener("change", () => { dateFrom = fromInput.value ? new Date(fromInput.value) : null; renderTable(); });
toInput.addEventListener("change", () => { dateTo = toInput.value ? new Date(toInput.value) : null; renderTable(); });
resetBtn.addEventListener("click", () => { setDefaultDates(); renderTable(); });
