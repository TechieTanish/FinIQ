...
document.querySelectorAll('.logo, .logo-text').forEach(el =>
...
// Popup and transaction handling
const popup = document.getElementById("transaction-popup");
const errorMsg = document.getElementById("error-msg");
let currentType = "income";

document.getElementById("add-income-btn").addEventListener("click", () => openPopup("income"));
document.getElementById("add-expense-btn").addEventListener("click", () => openPopup("expense"));
...
function openPopup(type) {
  ...
  document.getElementById("category").value = (categories[type] && categories[type][0]?.value) || "";
}
...
async function onSaveClick() {
  ...
  if (!amount) {
    errorMsg.textContent = "⚠️ Please enter amount";
    return;
  }
  try {
